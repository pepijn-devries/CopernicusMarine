...
[![Codecov test coverage](https://codecov.io/gh/pepijn-devries/CopernicusMarine/graph/badge.svg)](https://app.codecov.io/gh/pepijn-devries/CopernicusMarine)
<!-- badges: end -->
...
should consult `vignette("product-info")`.
...
```{r download-subset, eval=TRUE, fig.alt="Example plots of downloaded subsets", results='hide'}
my_data <-
  cms_download_subset(
...
In addition it is also possible to subset data with `stars_proxy` objects,
using either `cms_zarr_proxy()` or `cms_native_proxy()`. This is explained
in more detail in `vignette("proxy")`.
...
```{r native1, eval=requireNamespace("paws.storage")}
native_files <-
